...
SQInteger tool_sender(HSQUIRRELVM vm)
{
	...
	if (err && strcmp(err, "Error compiling string buffer")==0) {
		dbg->warning("tool_exec_script_t::sender", "error [%s] evaluating tool data sender", err);
		return SQ_ERROR;
	}
...
SQInteger two_click_tool_sender(HSQUIRRELVM vm)
{
	...
	if (err && strcmp(err, "Error compiling string buffer")==0) {
		dbg->warning("tool_exec_two_click_script_t::sender", "error [%s] evaluating two_click_tool data sender", err);
		return SQ_ERROR;
	}
